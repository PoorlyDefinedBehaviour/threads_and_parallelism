...
use threads_and_parallelism::matrix::{single_thread::multiply, Matrix};
...
pub fn criterion_benchmark(c: &mut Criterion) {
  ...
  let mut group = c.benchmark_group("single_thread_multiply_i64");
  ...
  let mut group = c.benchmark_group("single_thread_multiply_f64");
  for size in sizes.iter() {
    group.throughput(Throughput::Elements(*size as u64));
    group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
      b.iter(|| {
